...
def run_simulation(env, agent, num_games=1000):
    """
    Roda uma simulação e retorna as estatísticas de vitória, derrota (por tipo) e empate.
    """
    wins = 0
    losses = 0
    draws = 0
    ...
    for _ in range(num_games):
        ...
        # Analisa o resultado do jogo para contabilizar a derrota
        if reward > 0:
            wins += 1
        elif reward < 0:
            losses += 1
            if "estourou" in message:
                bust_losses += 1
            elif "Dealer venceu" in message:
                point_losses += 1
        else:
            draws += 1
            
    return wins, losses, draws, bust_losses, point_losses

def train_and_evaluate(hyperparams, seed=42):
    """
    Treina e avalia um agente com base em um conjunto de hiperparâmetros.
    Retorna a taxa de vitória, aproveitamento e o detalhamento das derrotas.
    """
    random.seed(seed)
    ...
    wins, losses, draws, bust_losses, point_losses = run_simulation(env, agent, num_games=1000)
...
if __name__ == "__main__":
    ...
    for config in test_configs:
        win_rate, wins, losses, draws, utilization_rate, bust_losses, point_losses = train_and_evaluate(config)
        results.append({
            'Config': config,
    ...
    header = f"{'ALPHA':<8} {'GAMMA':<8} {'EPS_DECAY':<12} {'EPISÓDIOS':<12} {'VITÓRIA %':<12} {'APROVEITAMENTO':<15} {'VITÓRIAS':<12} {'DERROTAS':<12} {'EMPATES':<12} {'ESTOUROS':<12} {'PONTOS':<12}"
    print(header)
    print("-" * 140)
    
    for res in results:
        config = res['Config']
        line = f"{config['alpha']:<8.2f} {config['gamma']:<8.2f} {config['epsilon_decay']:<12.4f} {config['num_episodes']:<12} {res['Vitória %']:<12} {res['Aproveitamento']:<15} {res['Vitórias']:<12} {res['Derrotas']:<12} {res['Empates']:<12} {res['Estouros']:<12} {res['Pontos']:<12}"
        print(line)
