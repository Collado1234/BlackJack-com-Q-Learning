    ...
    for _ in range(num_games):
        ...
        while not done:
            if policy == "q_learning":
                # Certifica-se de que a q_table existe e o estado está nela
                if agent and agent.q_table and state in agent.q_table:
                    action = np.argmax(agent.q_table[state])
                else:
                    action = random.choice([0, 1]) # fallback para a exploração
            elif policy == "random":
...
def train_and_evaluate(hyperparams, seed=42, num_eval_seeds=20, verbose=False):
    ...
    # Avaliação da política de Q-Learning
    ...
    # Avaliação da política Aleatória (um novo agente para não conflitar com o Q-learning)
    results_rand = []
    rand_agent = QLearningAgent(actions=[0,1], alpha=0, gamma=0) # Configuração para política aleatória
    for s in range(num_eval_seeds):
        env_eval = BlackjackEnv(seed=s)
...
def evaluate_casino_policy(num_eval_seeds=10):
    results_casino = []
    for s in range(num_eval_seeds):
        env_eval = BlackjackEnv(seed=s)
...
if __name__ == "__main__":
    ...
    # Avalia a política de Cassino uma vez para economizar tempo
    print("Avaliando a política de cassino...")
    avg_casino = evaluate_casino_policy(num_eval_seeds=10)
    print(f"Taxa de vitória do Cassino (média de 10 seeds): {avg_casino[0]:.2f}%")
    
    results = []
    for config in test_configs:
        avg_q, avg_rand, _, conf = train_and_evaluate(config, verbose=True)
        results.append((conf, avg_q, avg_rand, avg_casino))

    print("\n" + "="*150)
    print("        Relatório de Desempenho no Blackjack (MÉDIA de 10 seeds) - Q-Learning vs Aleatório vs Estratégia Cassino")
    print("="*150)
